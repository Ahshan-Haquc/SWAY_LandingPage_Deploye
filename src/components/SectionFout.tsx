    ...
    <div className={`flex flex-col justify-between items-center gap-3 p-6 ${className}`}>
      ...
      <p className="text-xs md:text-sm xl:text-xl text-center leading-relaxed">
        ...
        {item.linkText && (
          <a
            href={item.linkHref}
            className="underline font-medium hover:text-[#80b1fb]"
          >
            {item.linkText}
          </a>
        )}
      ...
      <button className="w-44 sm:w-64 mt-4 py-2 md:py-3 px-2 md:px-6 bg-[#FF6464] text-sm md:text-lg font-semibold rounded-full shadow-lg hover:bg-[#Faefb8] transition duration-300">
        BOOK
      </button>
      ...
      <Slider {...settings} className="w-full">
        {trainingData.map((item, index) => (
          <div className="h-fit w-fit p-1 md:p-4 overflow-hidden" key={index}>
            <div
              className="w-full h-screen md:h-[550px] xl:h-[750px] p-4 md:p-5 xl:p-10 bg-white border-0 md:border-2 md:border-gray-500 rounded-xl"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-12 h-full">
                ...
                <div className="relative w-full aspect-square my-auto pr-0 xl:pr-4">
                  ...
                  <div className="absolute inset-0 bg-black/70 opacity-0 hover:opacity-100 transition-opacity duration-500 flex flex-col justify-center items-center text-white p-6 rounded-lg md:hidden">
                    <h2 className="text-xl sm:text-2xl font-normal mb-2 text-center">
                      {item.title}
                    </h2>
                    <p className="text-sm mb-2 text-center">{item.subtitle}</p>
                    <p className="text-xs mb-3 text-center">
                      {item.description1.substring(0, 100)}...
                    </p>
                    <p className="text-xs mb-3 text-center">{item.price}</p>

                    <button className="px-6 py-2 bg-[#FF6464] text-white rounded-full text-sm font-semibold">
                      BOOK
                    </button>

                    <div className="flex space-x-2 text-xs pt-2">
                      {item.tags.map((tag, i) => (
                        <span key={i} className="border border-white px-2 py-1 rounded-full">
                          {tag}
                        </span>
                      ))}
                    </div>
